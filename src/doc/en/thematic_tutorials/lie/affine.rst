...
In this section and the next, we will also explain what facilities there are
in Sage for computing with these. We will often restrict ourselves to the case
...
`\alpha_i^\vee(\alpha_j)=a_{ij}`.  Moreover there exists a Kac-Moody Lie
algebra `\mathfrak{g}` containing `\mathfrak{h}` as an abelian subalgebra that
is generated by `\mathfrak{h}` and elements `e_i` and `f_i` such that
`[e_i,f_i]=\delta_{ij}\alpha_i^\vee`, `[h,e_i]=\alpha_i(h)e_i`,
`[h,f_i]=-\alpha_i(h)f_i`.  (These conditions do not quite characterize
`\mathfrak{g}`, but they do if supplemented by the Serre relations, which we
will not need or state.)
...
finite-dimensional simple Lie algebra `\mathfrak{g}^\circ`,
...
Cartan type is the extended Dynkin-diagram of `\mathfrak{g}^\circ`::
...
to an untwisted type (except `A_{2\ell}^{(2)}`).
For example the twisted type `['E',6,2]` in Aff2 is
...
When `V_\lambda\neq0`, the linear functional `\lambda` is called a *weight*.
...
case, if `\mathcal{g}` is an infinite Kac-Moody Lie algebra there are two
...
called *positive*, and those on `\mathbf{n}_-`,
called *negative*. 
