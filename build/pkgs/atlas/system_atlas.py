...
if os.environ.has_key('SAGE_ATLAS_LIB'):
    ...
    if os.path.isdir(ATLAS_LIB):
        ...
        has_atlas = os.path.exists(ATLAS_LIB+'/libatlas.so')
        has_lapack = os.path.exists(ATLAS_LIB+'/liblapack.so')
        has_cblas = os.path.exists(ATLAS_LIB+'/libcblas.so')
        has_f77blas = os.path.exists(ATLAS_LIB+'/libf77blas.so')

        if has_atlas == True and has_lapack == True and has_cblas == True and has_f77blas == True:

            s_gfortran = os.popen2('readelf -s ' +ATLAS_LIB+'/libf77blas.so | grep gfortran')[1].read()
            s_g95 = os.popen2('readelf -s ' + ATLAS_LIB + '/libf77blas.so | grep g95')[1].read()
            ...
            print("Set SAGE_ATLAS_LIB to a directory having liblapack.so,libcblas.so,libatlas.so,libf77blas.so,")
