...
class AffineCurve(Curve_generic, AlgebraicScheme_subscheme_affine):
    ...
    def intersection_multiplicity(self, C, P):
        ...
        ::
            ...
            sage: A.<x,y,z> = AffineSpace(QQ,3)
            sage: C = Curve([y^2 + x + z, y^2 - z^2 + x], A)
            sage: D = Curve([y + z^3 - z^2 + x, y^3 + x + z^3], A)
            sage: Q1 = A([-1,1,0])
            ...
            Traceback (most recent call last):
            ...
            TypeError: (=(1, 1, 1)) must be a point in the intersection of this
            curve with (=Affine Curve over Rational Field defined by z^3 - z^2 + x +
            y, y^3 + z^3 + x)
        ...
        ::
            ...
            Traceback (most recent call last):
            ...
            TypeError: irreducible components of the intersection of this curve and
            (=Affine Plane Curve over Finite Field of size 7 defined by -x^3 + y^3)
            containing (=(1, 1)) must have dimension zero
        """
        if not self.intersects_at(C, P):
            raise TypeError("(=%s) must be a point in the intersection of this curve with (=%s)"%(P, C))
        T = self.intersection(C).irreducible_components()
        for Y in T:
            tmp = None
            try:
                tmp = Y(P)
            ...
            if not tmp is None:
                if Y.dimension() > 0:
                    raise TypeError("irreducible components of the intersection of this curve and (=%s) " \
                                    "containing (=%s) must have dimension zero"%(self,P))
        AA = self.ambient_space()
        # polynomials defining intersection
        polys = list(self.defining_polynomials())
        polys.extend(list(C.defining_polynomials()))
        # move P to the origin
        chng_coords = [AA.gens()[i] + P[i] for i in range(AA.dimension_relative())]
        R = AA.coordinate_ring().change_ring(order="negdegrevlex")
        I = R.ideal([f(chng_coords) for f in polys])
        return singular.vdim(singular.std(I)).sage()
