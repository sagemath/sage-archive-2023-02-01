...
class PermutationGroup_generic(group.FiniteGroup):
    ...
    def _regular_subgroup_gap(self):
        ...
        """ % (self._gap_().name(), len(self.domain())))
    ...
    @cached_method
    def regular_subgroup(self, return_group = True):
        r"""
        Return a regular subgroup if there is one.

        INPUT:

        - ``return_group`` (boolean) -- If ``return_group = True`` (default),
          a regular subgroup is returned if there is one, and ``None`` if
          there isn't. When ``return_group = False``, only a boolean
          indicating whether such a group exists is returned instead.
