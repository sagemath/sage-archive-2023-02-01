...
class SchemeMorphism_polynomial_projective_space(SchemeMorphism_polynomial):
    ...
    def nth_iterate(self,P,n,normalize=False):
        ...
        this function returns the nth iterate of `P` by ``self``. If ``normalize=True``, then the coordinates
    ...
    def degree(self):
        ...
        The degree is defined as the degree of the homogeneous polynomials that are the
        coordinates of ``self``.
    ...
    def dehomogenize(self,n):
        ...
        - :class:`SchemeMorpism_polynomial_affine_space` (on nth affine patch)
    ...
    def conjugate(self,M):
        r"""
        Conjugates ``self`` by ``M``, i.e. `M^{-1} \circ f \circ M`. If possible the map will be defined over the
