...
cdef inline bint overunderflow(long* ordp, celement unit, PowComputer_class prime_pow):
    """
...
cdef inline bint overunderlow_mpz(long* ordp, mpz_t ordp_mpz, celement unit, PowComputer_class prime_pow):
    """
...
cdef class FPElement(pAdicTemplateElement):
    ...
    def add_bigoh(self, absprec):
        ...
        if absprec is inifinity or very_neg_val(self.ordp):
            return self
    ...
    def list(self, lift_mode = 'simple', start_val = None):
        ...
        if (self.prime_pow.in_field and self.ordp > 0) or start_val is not None:
            if lift_mode = 'teichmuller':
                zero = R(0)
            else:
    ...
    cpdef pAdicTemplateElement unit_part(FPElement self):
        ...
        """
        if huge_val(self.ordp):
            raise ValueError("unit part of 0 and infinitynot defined")
...
cdef class pAdicCoercion_ZZ_FP(RingHomomorphism_coercion):
    ...
    cpdef Element _call_(self, x):
        ...
        EXAMPLES::
            ...
            sage: f = ZpFP(5).coerce_map_from(ZZ)
            sage: f(0).parent()
            5-adic Ring of floating point 5^20
            sage: f(5)
            5 + O(5^20)
        """
...
cdef class pAdicConvert_FP_ZZ(RingMap):
    ...
    EXAMPLES::
        ...
        Set-theoretic ring morphism:
          From: 5-adic Ring of floating point 5^20
          To:   Integer Ring
    """
    def __init__(self, R):
        """
    ...
    cpdef Element _call_(self, _x):
        ...
        cdef Integer ans = PY_NEW(Integer)
        cdef FPElement x = _x
        cconv_mpz_t_out(ans.unit, x.unit, 0, x.prime_pow.prec_cap, x.prime_pow)
        return ans

cdef class pAdicConvert_QQ_FP(Morphism):
    """
    The inclusion map from QQ to a floating point ring that is defined
    ...
    cpdef Element _call_(self, x):
        ...
        EXAMPLES::
            ...
            O(5^4)
        """
        if mpq_sgn((<Rational>x).unit) == 0:
            return self._zero
        cdef FPElement ans = self._zero._new_c()
        cconv_mpq_t(ans.unit, (<Rational>x).unit, ans.prime_pow.prec_cap, True, ans.prime_pow)
        return ans
...
def unpickle_fpe_v2(cls, parent, unit, ordp):
    ...
    EXAMPLES::
        ...
        sage: R = ZpFP(5)
        sage: a = unpickle_fpe_v2(pAdicFloatingPointElement, R, 17*25); a
        2*5^2 + 3*5^3 + O(5^20)
        sage: a.parent() is R
        True
    """
