...
class EllipticCurve_number_field(EllipticCurve_field):
    ...
    def saturation(self, points, verbose=False,
        ...
        -  ``lower_ht_bound (real)`` - (default: None), lower bound of the
        ...
        by the input points in their saturation
...
def p_saturation(Plist, p, sieve=True, lin_combs = dict(), verbose=False):
    ...
    subgrouyp generated contains that generated by ``Plist`` with
    ...
    Note that the generators for the reductions of E modulo prime
    ideals will vary from run to run as they involve random choices::

        sage: p_saturation([P,Q,R],19, verbose=True) # random
        Using sieve method to saturate...
        There is 19-torsion modulo Fractional ideal (i + 14), projecting points
         --> [(184 : 27 : 1), (0 : 0 : 1), (196 : 1 : 1)]
        19-rank of reduction is 1, using discrete logs
         --adding row (16, 16, 2)
         --rank is now 1
        There is 19-torsion modulo Fractional ideal (i - 14), projecting points
    ...
    def projections(Q, p):
        ...
        relevant_gens = [(n,G.gen(i)) for i,n in enumerate(G.generator_orders()) if p.divides(n)]

        # if p-rank of the reduction is 1, use discrete logs:
        from sage.groups.generic import discrete_log_lambda
        if len(relevant_gens)==1:
            if verbose:
                print("%s-rank of reduction is 1, using discrete logs" % p)
            n,g = relevant_gens[0]
            if G.ngens()>1:
                # multiply by the order of the other generator, which
                # is coprime to p, to map into the cyclic subgroup
                # whose order is divisible by p:
                m = G.generator_orders()[0]
                if p.divides(m):
                    m = G.generator_orders()[1]
                n = n//m
                g = m*g
            else:
                m = 1
            v = [discrete_log_lambda(m*pt,g,(0,n),'+') for pt in projPlist]
            return [vector(GF(p),v)]

        # now the p-rank is 2 and we use Weil pairing:
        if len(relevant_gens)==2:
            if verbose:
                print("%s-rank of reduction is 2, using Weil pairing" % p)
            nn = [r[0] for r in relevant_gens]
            mm = [n.prime_to_m_part(p) for n in nn]
            nn = [n//m for n,m in zip(nn,mm)] # p-powers
            pp = max(nn) # p-exponent of G; use Weil pairings of this order
            gg = [r[1] for r in relevant_gens]
            gg = [(m*g).element() for m,g in zip(mm,gg)] # p-power order gens

            ml = max(mm) # prime-to-p exponent of G: multiply by this
                         # to map into the p-primary part.
            # a pp'th root of unity:
            zeta = gg[0].weil_pairing(gg[1],pp)
            # for i=0, 1 these are the homomorphisms from E to F_p:
            def a(pt,i):
                w = (ml*pt).weil_pairing(gg[i],pp)
                return discrete_log_lambda(w,zeta,(0,pp),'*') % p
            return [vector(GF(p), [a(pt,i) for pt in projPlist]) for i in [0,1]]

        # alternative which works sometimes
        # ans = []
        # for i,m in enumerate(G.invariants()):
        #     if p.divides(m):
        #         cof = m//p
        #         v = [G.coordinate_vector(cof*pt)[i]//cof for pt in projPlist]
        #         ans += [vector(GF(p),v)]
        # return ans
