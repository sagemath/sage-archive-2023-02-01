...
cdef class interval_bernstein_polynomial_integer(interval_bernstein_polynomial):
    ...
    EXAMPLES:
        ...
        sage: str(bp)
        '<IBP: ((-3, -1, 1, -1, -3, -1) + [0 .. 2)) * 2^-3 over [1 .. 5/4]; usign 1; level 2; slope_err [3.1415926535897931 .. 3.1415926535897936]>'
        sage: bp.variations()
        (3, 3)
    """
    ...
    def __init__(self, Vector_integer_dense coeffs, Rational lower, Rational upper, int lsign, int usign, int error, int scale_log2, int level, RealIntervalFieldElement slope_err):
        ...
        EXAMPLES:
            ...
            sage: str(bp)
            '<IBP: ((50, -30, -10) + [0 .. 17)) * 2^3 over [-3/7 .. 4/7]; usign -1; level 2; slope_err [9.9999999999999990e-31 .. 1.0000000000000001e-30]>'
        """
        assert(len(coeffs) > 0)
        self.coeffs = coeffs
        self.lower = lower
...
#         vn = dprod_matrow_vec(downmat, self.coeffs, next)
    ...
    def down_degree_iter(self, context ctx, max_scale):
        ...
        EXAMPLES:
            ...
            sage: str(rbp)
            '<IBP: (-4, 249, 2497) + [0 .. 9); level 2; slope_err [-114.00000000000000 .. 114.00000000000000]>'
        """
...
cdef class interval_bernstein_polynomial_float(interval_bernstein_polynomial):
    ...
    EXAMPLES:
        ...
        sage: str(bp)
        '<IBP: ((-0.3, -0.1, 0.1, -0.1, -0.3, -0.1) + [0.0 .. 0.2]) * 2^-3 over [1 .. 5/4]; usign 1; level 2; slope_err [3.1415926535897931 .. 3.1415926535897936]>'
        sage: bp.variations()
        (3, 3)
    """
    ...
    def __init__(self, RealDoubleVectorSpaceElement coeffs, Rational lower, Rational upper, int lsign, int usign, double neg_err, double pos_err, int scale_log2, int level, RealIntervalFieldElement slope_err):
        ...
        EXAMPLES:
            ...
            degree 2 IBP with floating-point coefficients
            sage: str(bp)
            '<IBP: ((0.5, -0.3, -0.1) + [-0.02 .. 0.17]) * 2^3 over [-3/7 .. 4/7]; usign -1; level 2; slope_err [9.9999999999999990e-31 .. 1.0000000000000001e-30]>'
        """
        assert(len(coeffs) > 0)
        self.coeffs = coeffs
        self.lower = lower
...
def real_roots(p, bounds=None, seed=None, skip_squarefree=False, do_logging=False, wordsize=32, retval='rational', strategy=None):
    ...
    hashing the input polynomial, so that results are repeatable.)  The
