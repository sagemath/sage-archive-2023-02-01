...
setup(ext_modules = ext_modules,
    ...
    if os.system(cmd):
        ...
        raise RuntimeError("Error converting {0} to C:\n{1}\n{2}".format(filename, log, err))
...
##         open(target_c,'w').write(R)
    ...
    if os.system(cmd):
        ...
        raise RuntimeError("Error compiling {0}:\n{1}\n{2}".format(filename, log, err))
    ...
    if verbose:
        print(cmd)
    if os.system(cmd):
        raise RuntimeError("Error copying extension module for {0}".format(filename))
    ...
    if create_local_so_file:
        ...
        if os.system(cmd):
            raise RuntimeError("Error making local copy of shared object library for {0}".format(filename))
