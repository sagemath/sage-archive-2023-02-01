...
cdef int vertex_separation_BAB_C(binary_matrix_t H,
    ...
    if level<=max_prefix_length and current_cost<upper_bound and len(prefix_storage)<max_prefix_number:
        ...
        if loc_level>level:
            # We also store the prefixes generated by the greedy steps
            delta = list(frozen_prefix)
            for i in range(level+1, min(loc_level, max_prefix_length)+1):
                delta.append(prefix[i])
                prefix_storage.add(frozenset(delta))
