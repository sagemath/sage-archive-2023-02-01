...
cdef class gen(sage.structure.element.RingElement):
    ...
    def galoisinit(self, den=None):
        """
        Calculate Galois group of number field or polynomial K.
        Wraps the `galoisinit`_ function from PARI.

        INPUT:

        - ``self`` -- a number field or a polynomial.

        - ``den`` -- if set, it must be a suitable value for the
          denominator of the inverse Vandermonde matrix.
        ...
        Note that the element indices in Sage are zero-based.
    ...
    def galoispermtopol(self, perm):
        """
        Polynomial defining a given Galois automorphism.
        Wraps the `galoispermtopol`_ function from PARI.
    ...
    def galoisfixedfield(self, perm, long flag=0, v=-1):
        """
        Computes the fixed field of a galois group.
        Wraps the `galoisfixedfield`_ function from PARI.

        INPUT:

        - ``self`` -- A Galois group as generated by ``galoisinit``.

        - ``perm`` -- An element of a galois group, a vector of such elements,
          or a subgroup generated by ``galoissubgroups``.

        - ``flag`` -- Amount of data to include in output.

        - ``v`` -- Name of the second variable to use, defaults to ``y``.

        OUTPUT:

        If ``flag`` is zero, the output is a two-element tuple
        consisting of the defining polynomial of the fixed field
        and a descrption of its roots modulo the ptimes used in the group.
        If ``flag`` is one, just the polynomial is returned.
        If ``flag`` is two, a third tuple element will describe the
        factorization of the original polynomial
        using the variable indicated by ``v``
        to stand for a root of the polynomial from the first tuple element.
    ...
    def galoissubfields(self, long flag=0, v=-1):
        """
        List all subfields of a given Galois group.
        Wraps the `galoissubfields`_ function from PARI.
        This methos is essentially the same as applying ``galoisfixedfield``
        to each group returned by ``galoissubgroups``.

        INPUT:

        - ``self`` -- A Galois group as generated by ``galoisinit``.

        - ``flag`` -- Has the same meaning it has in ``galoisfixedfield``.

        - ``v`` -- Has the same meaning it has in ``galoisfixedfield``.

        OUTPUT:

        A vector of all subfields of this group.
        Each entry is as described in the ``galoisfixedfield`` method.
    ...
    def galoissubgroups(self):
        """
        List all subgroups of a given Galois group.
        Wraps the `galoissubgroups`_ function from PARI.

        INPUT:

        - ``self`` -- A Galois group as generated by ``galoisinit``,
          or a subgroup thereof returned by ``galoissubgroups``.
    ...
    def galoisisabelian(self, long flag=0):
        """
        Decide whether this is an abelian group.
        Wraps the `galoisisabelian`_ function from PARI.
        ...
        Zero if this is not an abelian group. If it is, then the output
        depends on ``flag``: if it is one, the return value is simply one.
        If the flag is zero, then the HNF matrix of ``self`` over its
    ...
    def galoisisnormal(self, subgrp):
        """
        Decide whether the given group is a normal subgroup of this group.
        Wraps the `galoisisnormal`_ function from PARI.
        ...
        - ``subgrp`` -- A subgroup of ``self``, returned by ``galoissubgroups``.

        OUTPUT:

        One if the given group is a subgroup of this one, zero otherwise.
