...
class UnramifiedExtensionCappedRelativeElement(pAdicRingGenericElement):
    def __init__(self, parent, x, absprec = infinity, relprec = infinity, check = True, construct = False):
        ...
        if isinstance(x, UnramifiedRingExtensionCappedRelativeElement) and x.parent() is self.parent():
        ...
        if isinstance(x, list):
            if len(x) == parent.degree():
                ...
                self._unit = UnramifiedExtensionElement(parent, x) # the parent here makes my head hurt a bit.
                self._ordp = val
                return
        #We now try casting into the base ring
        x = parent.base_ring()(x, absprec = absprec, relprec = relprec)
        self._ordp = x.valuation()
        self._unit = UnramifiedExtensionElement(parent, parent.polynomial_ring()(x.__rshift__(self._ordp)), construct = True)

    def _neg_(self):
        return UnramifiedRingExtensionCappedRelativeElement(self.parent(), (self._ordp, -self._unit), construct = True)
    ...
    def _add_(self, right):
        ...
        return UnramifiedRingExtensionCappedRelativeElement(self.parent(), (self._ordp * right, -self._unit + right._unit), construct = True)
