...
Sage имеет обширную функциональность в плане теории чисел. Например,
можно производить арифметические операции в :math:`\ZZ/N\ZZ`:
...
Функция ``sigma(n,k)`` в Sage суммирует :math:`k`-е степени делителей ``n``:
...
Далее покажем алгоритм Эвклида, :math:`\phi`-функцию Эйлера и китайскую
теорему об остатках:
...
:math:`p`--адические числа
--------------------------

Поле :math:`p`-адических чисел реализовано в Sage. Обратите внимание:
как только поле :math:`p`-адических чисел создано, его точность не
может быть изменена.
