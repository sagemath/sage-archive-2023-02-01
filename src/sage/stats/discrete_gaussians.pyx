...
from sage.rings.real_mpfr cimport RealNumber, mpfr_get_prec
...
cdef class BernoulliSampler(BernoulliBase):
    def __init__(self, c):
        ...
        BernoulliBase.__init__(self)
        self._gen = dgs_bern_init((<RealNumber>c).value)

    cpdef int raw_call(self):
        cdef randstate rstate = current_randstate()
        return dgs_bern_call(self._gen, rstate.gmp_state)
        
    def __clear__(self):
        if self._gen:
            dgs_bern_clear(self._gen)
...
cdef class BernoulliExpSampler(BernoulliBase):
    def __init__(self, f, l=None):
        ...
        if l is None:
            l = 0
        self._gen = dgs_bern_exp_init((<RealNumber>f).value, l)

    cpdef int raw_call(self, x):
        cdef randstate rstate = current_randstate()
        if not isinstance(x, Integer):
            x = Integer(x)
        return dgs_bern_exp_call(self._gen, (<Integer>x).value, rstate.gmp_state)
        
    def __clear__(self):
        if self._gen:
            dgs_bern_exp_clear(self._gen)
