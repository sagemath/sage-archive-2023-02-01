...
class SineGordonYsystem(SageObject):
    ...
    def __init__(self, X, na):
        ...
        self._type = X
        if na[0] <= 2:
            raise ValueError("The first integer in the defining sequence must be greater than 2.")
        if any(x not in NN for x in na):
            raise ValueError("The defining sequence must contain only positive integers.")
        self._na = tuple(na)
        if self._na == (3,) and self._type == 'A':
            raise ValueError("The algorithm gives fuzzy results with the integer sequence (3,) in type 'A', do you really need sage to plot a square?")
    ...
    def na(self):
        r"""
        Returns the sequence of the integers `n_a` defining ``self``.
    ...
    def rk(self):
        r"""
        Returns the sequence of integers ``r^{(k)}``, i.e. the width of an interval
        of type 'L' or 'R' in the ``k``-th generation.
    ...
    def pa(self):
        r"""
        Returns the sequence of integers  ``p_a``, i.e. the total number of
    ...
    def qa(self):
        r"""
        Returns the sequence of integers  ``q_a``, i.e. the total number of
    ...
    def r(self):
        r"""
        Returns the number of vertices in the polygon realizing ``self``.
    ...
    def vertices(self):
        r"""
        Returns the vertices of the polygon realizing ``self`` as the ring of
    ...
    def triangulation(self):
        r"""
        Returns the initial triangulation of the polygon realizing ``self`` as a
    ...
    def intervals(self):
        r"""
        Returns, divided by generation, the list of intervals used to construct
        the initial triangulation. Each such interval is a triple ``(p, q, X)`` 
        where ``p`` and ``q`` are the two extremal vertices of the interval and 
        ``X`` is the type of the interval (one of 'L', 'R', 'NL', 'NR').
    ...
    def plot(self, **kwds):
        ...
        def plot_arc(radius, p, q, **opts):
            ...
            if self.type() == 'A':
                ...
                def f(t):
                    return (radius_arc*cos(t) + center[0], radius_arc*sin(t) + center[1])
                return parametric_plot(f(t), (t, center_angle_p, center_angle_q), **opts)
            elif self.type() == 'D':
                if p >= q:
                    q += self.r()
                ...
                def f(t):
                    return exp(I*((cos(t)+I*sin(t))*arc_radius+arc_center)) * radius
                return parametric_plot((real_part(f(t)), imag_part(f(t))), (t, 0, pi), **opts)
        ...
        neuter_intervals = [x for x in flatten(self.intervals()[:-1], max_level=1) if x[2] in ["NR", "NL"] ]
        shaded_triangles = map(triangle, neuter_intervals)
        for (p, q, r) in shaded_triangles:
            points = list(plot_arc(radius, p, q)[0]) + list(plot_arc(radius, q, r)[0]) + list(reversed(plot_arc(radius, p, r)[0]))
        ...
        if self.type() == 'D':
            s = radius / 50.0
            P += polygon2d([(s,5*s), (s,7*s), (3*s,5*s), (3*s,7*s)], color=triangulation_opts['color'])
            P += bezier_path([[(0,0), (2*s,1*s), (2*s,6*s)], [(2*s,10*s), (s,20*s)], [(0,30*s), (0,radius)]], **triangulation_opts)
            P += bezier_path([[(0,0), (-2*s,1*s), (-2*s,6*s)], [(-2*s,10*s), (-s,20*s)], [(0,30*s), (0,radius)]], **triangulation_opts)
            P += point((0,0), zorder=len(P), **points_opts)
        # Vertices        
        v_points = dict([[x, (radius * cos(vertex_to_angle(x)), radius * sin(vertex_to_angle(x)))] for x in self.vertices()])
        for v in v_points:
            P += point(v_points[v], zorder=len(P), **points_opts)
        # Reflection axes
        P += line([(0, 1.1*radius), (0, -1.1*radius)], zorder=len(P), **reflections_opts)
