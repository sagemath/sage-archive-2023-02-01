...
#            tester.assertEqual(self(an_element), an_element, "element construction is not idempotent")
        ...
        def algebra(self, base_ring, category = None):
            ...
            that of `S`. (Note that the ``category`` keyword here needs
            to be fed with the structure on `S` to be used, not the
            structure that one wants to obtain on the result -- see
            examples below.)
            ...
            Despite saying "free module", this is an algebra and its
