...
class WordGenerator(object):
    ...
    def ThueMorseWord(self, alphabet=(0, 1)):
        ...
        We use the following definition: $t[n]$ is the sum modulo 2 of
        the digits in the binary expansion of $n$.
    ...
    def ChristoffelWord(self, p, q, alphabet=(0,1)):
        ...
        relatively prime non-negative integers, over the given two-letter alphabet.

        The \emph{Christoffel word of slope $p/q$} is obtained from the Cayley
        graph of $\mathbb{Z}/(p+q)\mathbb{Z}$ with generator $q$ as follows.
        If $u \rightarrow v$ is an edge in the Cayley graph, then
        $v = u + p \mod{p+q}$. Label the edge $u \rightarrow v$ by alphabet[1]
        if $u < v$ and alphabet[0] otherwise. The Christoffel word is the word
        obtained by reading the edge labels along the cycle beginning from 0.
    ...
    def UpperChristoffelWord(self, p, q, alphabet=(0,1)):
        ...
        relatively prime non-negative integers, over the given alphabet.

        The \emph{upper Christoffel word of slope $p/q$} is equal to the
        reversal of the lower Christoffel word of slope $p/q$. Equivalently, if
        $xuy$ is the lower Christoffel word of slope $p/q$, where $x$ and $y$
        are letters, then $yux$ is the upper Christoffel word of slope
    ...
    def FixedPointOfMorphism(self, morphism, first_letter):
        ...
        A \emph{fixed point} of a morphism $\varphi$ is a word $w$ such that
        $\varphi(w) = w$.
    ...
    def CodingOfRotationWord(self, alpha, beta, x=0, alphabet=(0,1)):
        ...
        The \emph{coding of rotation} corresponding to the parameters
    ...
    def CharacteristicSturmianWord(self, cf, alphabet=(0, 1), repeat=True):
        ...
        alphabet with slope given by the continued fraction [0, cf[1], cf[2], ...].
        Here cf should be an iterator.

        The \emph{characteristic Sturmian word of slope}
        ...
        of slope $\alpha$ is $\lfloor\alpha(n+1)\rfloor - \lfloor\alpha n\rfloor + \lfloor\alpha\rfloor$.
    ...
    def StandardEpisturmianWord(self, directive_word):
        ...
        \emph{standard episturmian} (or \emph{epistandard}) iff there exists an
        infinite word $x_1x_2x_3\cdots$ over $A$ (called the \emph{directive
        ...
        Note that an infinite word is \emph{episturmian} if it has the same set
        of factors as some epistandard word.

        See for instance [1], [2], and [3].

        INPUT:
            directive word -- an infinite word or a period of a periodic
            infinite word
    ...
    def _StandardEpisturmianWord_LetterIterator(self, directive_word):
        ...
        INPUT:
            directive word -- an infinite word or a finite word. If
    ...
    def RandomWord(self, n, m=2, alphabet=None):
        ...
        INPUT:
            ...
            alphabet -- any container of length m that is suitable to
                build an instance of OrderedAlphabet (list, tuple, str, ...)
...
class ChristoffelWord_Lower(FiniteWord_over_OrderedAlphabet):
    def __init__(self, p, q, alphabet=(0,1)):
        ...
        The \emph{Christoffel word of slope $p/q$} is obtained from the
        Cayley graph of $\mathbb{Z}/(p+q)\mathbb{Z}$ with generator $q$ as
    ...
    def markoff_number(self):
        ...
        The \emph{Markoff number} of a Christoffel word $w$ is $1/3 trace M(w)$,
    ...
    def standard_factorization(self):
        ...
        The \emph{standard factorization} of a Christoffel word $w$ is the
