...
To refresh the autogenerated files you need to install the TOPCOM
...
In short, you need to do the following. First, install the spkg:
  ...
  [user@localhost]$ sage -pkg TOPCOM-<version>
  [user@localhost]$ sage -i -s TOPCOM-<version>.spkg
  ... does not matter if it actually compiles ...

Then, rerun the autotools:

  [user@localhost]$ cd $SAGE_LOCAL/spkg/build/TOPCOM-<version>/src
  [user@localhost]$ autoreconf -fiv

Finally, copy the newly generated files:

  [user@localhost]$ cd TOPCOM-<version>/patches/
  [user@localhost]$ ./refresh_autogenerated.sh $SAGE_LOCAL/spkg/build/TOPCOM-<version>/src
EOF
echo
