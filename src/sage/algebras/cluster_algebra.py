...
class ClusterAlgebra(Parent):
    ...
    def mutate_initial_parallel(self, k, inplace=True, mutating_F=True):
        if inplace:
            algebra = self
        else:
            algebra = copy(self)
        ...
        def mutation_helper(g_vect):
            ...
            new_F = algebra._F_poly_dict[g_vect](F_subs_tuple)*algebra._U.gen(k)**h*(algebra._U.gen(k)+1)**g_vect[k]
          
            return [tuple(new_g_vect), new_path, new_F]
        
        new_data = mutation_helper([(g_vect,) for g_vect in algebra._path_dict])
        new_path_dict = dict()
        new_F_poly_dict = dict()
        for _,(new_g_vect, new_path, new_F) in new_data:
            new_path_dict[new_g_vect] = new_path
            new_F_poly_dict[new_g_vect] = new_F
        algebra._path_dict = new_path_dict
        algebra._F_poly_dict = new_F_poly_dict
        algebra._path_dict[tuple(identity_matrix(n).column(k))] = []
        algebra._F_poly_dict[tuple(identity_matrix(n).column(k))] = self._U(1)
