...
def IntegralLattice(inner_product_matrix,basis=None,already_echelonized=False,check=True):
    ...
    return(FreeQuadraticModule_symmetric_integral(ambient=FreeQuadraticModule(ZZ, inner_product_matrix.ncols(), inner_product_matrix=inner_product_matrix),basis=basis,inner_product_matrix=inner_product_matrix,already_echelonized=False))
...
class FreeQuadraticModule_symmetric(FreeQuadraticModule_submodule_with_basis_pid):
    """
    This class represents free quadratic ZZ-modules with a symmetric and non-degenerate inner product.
    """
    def __init__(self, ambient, basis, inner_product_matrix, check=True, already_echelonized=False):
        """
        Create the symmetric quadratic module spanned by basis in the ambient space.
        ...
        - ``inner_product_matrix`` - a symmetric matrix over the base ring
        ...
        if self.inner_product_matrix()!=self.inner_product_matrix().transpose():
            raise ValueError("The inner product matrix \n %r \n must be symmetric." %self.inner_product_matrix())
    ...
    def _repr_(self):
        ...
        """
        if self.is_sparse():
            s = "Sparse symmetric free quadratic module of degree %s and rank %s over %s\n"%(
                self.degree(), self.rank(), self.base_ring()) + \
                "Basis matrix:\n%r\n" % self.basis_matrix() + \
                "Inner product matrix:\n%r" % self.inner_product_matrix()
        else:
            s = "Symmetric free quadratic module of degree %s and rank %s over %s\n"%(
                self.degree(), self.rank(), self.base_ring()) + \
                "Basis matrix:\n%r\n" % self.basis_matrix() + \
                "Inner product matrix:\n%r" % self.inner_product_matrix()
        return s
    ...
    def is_integral(self):
        """
        Return True if the Gram matrix of self has coefficients over the base ring. 
        """
        denom = self.gram_matrix().denominator()
        denom = self.base_ring()(denom)
        return(denom.is_unit())
    
    def is_even(self):
        """
        Return True if the diagonal entries of the Gram matrix of self are even.
        
        An element is considered even if it is divisible by 2 over the base ring. 
        ...
        """
        if self.is_integral():
            d = gcd(self.gram_matrix().diagonal())
            return(self.base_ring()(2).divides(d))
        else:
            return(False)
    ...
    def signature_vector(self):
        ...
        * z = number of zero eigenvalues
        ...
        for the Gram matrix associated to Q.
    ...
    def span_of_basis(self, basis, check=True, already_echelonized=False):
        r"""
        Return the symmetric free Quadratic R-module with the given basis, 
        where R is the base ring of self.  Note that this R-module need not
        ...
        """
        return FreeQuadraticModule_symmetric(
...
class FreeQuadraticModule_symmetric_integral(FreeQuadraticModule_symmetric):
    """
    ...
    def dual(self):
        ...
        Since lattices are integral, they are contained in their dual::
        
            sage: Ldual.is_submodule(L)
            True      
        """
        return(self.span_of_basis(self.gram_matrix().inverse()*self.basis_matrix()))
    ...
    def direct_sum(self,M):
        ...
        return(FreeQuadraticModule_symmetric_integral(ambient=ambient,basis=basis,inner_product_matrix=ambient.inner_product_matrix(),already_echelonized=False))
    ...
    def sublattice(self,basis):
        ...
        """
        M = FreeQuadraticModule_symmetric_integral(ambient=self.ambient_module(),basis=basis,inner_product_matrix=self.inner_product_matrix(),already_echelonized=False)
    ...
    def overlattice(self,gens):
        ...
        L = FreeQuadraticModule_symmetric_integral(ambient=self.ambient_module(),basis=basis,inner_product_matrix=self.inner_product_matrix(),already_echelonized=False)
