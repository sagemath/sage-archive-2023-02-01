...
cdef class TensorProductOfQueerSuperCrystalsElement(TensorProductOfRegularCrystalsElement):
    ...
    [GJK+2014]_. Given crystals `B_1` and `B_2` of type `Q(n+1)`, we define 
    the tensor product `b_1\otimes b_2\in B_1\otimes B_2` where `b_1\in B_1` 
    and `b_2\in B_2` as the following:
    ...
    Here, `w_i=s_2\ldots s_i s_1\ldots s_{i-1}` and `s_i` is the reflection 
