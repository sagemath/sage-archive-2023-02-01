...
def one_radical_difference_family(K, k):
    ...
    Let `H = K^*` be the multiplicative group of invertible elements in the
    finite field `K` and `x` a generator of `H`. We denote by `H^j` the set of
    `j`-th power in `H`. In order to treat uniformly the case of `k` odd and `k`
    even, let us introduce some notations.
    ...
    Note that `r` is a primitive `k`-th root of unity for `k` odd and a `k-1`-th
    root of unity for `k` even. Moreover `H^{2mt} = \langle r \rangle`.

    It is easy to check that for `k` odd `\Delta H^{2mt} =  A H^{mt}` while for
    `k` even `\Delta (H^{2mt} \cup \{0\}) = A H^{mt}`.  Here `\Delta B` is the
    set of differences of distinct elements in `B` (see [Wi72]_).

    From now on, we consider the case of `k` odd, the case of `k` even being
    ...
    of `H / H^{mt}`. One can consider this as a tiling (or packing) of `H /
    H^{mt}` by translates of the unique tile `A`. This tiling problem is solved
    with :func:`one_cyclic_tiling`.
    """
    q = K.cardinality()
    x = K.multiplicative_generator()
