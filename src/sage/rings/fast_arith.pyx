...
cpdef prime_range(start, stop=None, algorithm="pari_primes", bint py_ints=False):
    ...
    - ``algorithm`` -- optional string (default: "pari_primes"), one of:

        - "pari_primes": Uses PARI's :pari:`primes` function to generate all primes
          from 2 to stop if stop <= 436273009 (approximately 4.36E8). (Otherwise uses
          algorithm "pari_isprime".) This is fast but may crash if there is insufficient
          memory (and will not be used when stop > 436273009).
