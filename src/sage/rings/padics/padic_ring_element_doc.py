...
There are three types of $p$-adic integer elements belonging to two different rings.
First, the general $p$-adic integer ring has both finite and arbitrary precision elements.

A finite precision element is stored as a product $p^r \cdot u$, where $r$ is a
non-negative integer and $u$ is a power series in $p^k$, an appropriate sized power of
$p$.  Note that $u$ is not guaranteed to be a unit in the internal representation.

An arbitrary precision element is stored as a lazy element, ie as a function that takes
a precision $n$ as input and returns a finite precision element of that precision as
output.  There are some issues with the halting problem which need to be addressed.
As a subclass, we have rational elements, which are lazy element wrappers around
rational numbers.

Finally, there's a fixed modulus ring of $p$-adic integers.  At creation this modulus
is fixed, and elements are stored as an element of $\Z / p^n \Z$ together with a
precision which is less than or equal to $n$.  Since this ring is basically a wrapper
around $\Z / p^n \Z$, arithmetic will usually be faster than in the general $p$-adic ring.

Automatic casting works as follows:
A rational number cast into a general $p$-adic ring will be cast into the rational subclass
of the lazy element class.
O(p^k) will create a finite precision element congruent to $0$ modulo $p^k$
Adding or multiplying two finite precision elements will give another finite precision element
whose precision is the minimum of the precisions of the inputs (note that addition does not
guarantee the "unit" is actually a unit.  Multiplication does, because in order to ensure
that precision behaves correctly it must normalize the units before multiplying).
Inversion preserves precision.
Adding or multiplying two arbitrary precision elements will give another arbitrary precision
element.  In the case of a rational element, it will actually perform the rational addition.
Normal arbitrary precision elements are just added as functions.
Adding or multiplying an arbitrary precision element with a finite precision element will
give a finite precision element of the same precision as the input finite precision element.
Adding or multiplying two fixed modulus elements gives another fixed modulus element.
This operation will be implemented in $\Z / p^n \Z$ and thus will be fast.  Precision is
the minimum of the precisions of the inputs (note that this might not be the full $n$).
Adding or multiplying a finite precision element by a fixed modulus element gives a
fixed modulus element, with precision possibly less than the modulus.
Adding or multiplying an arbitrary precision element by a fixed modulus element will give a fixed
modulus element with precision equal to the input.

To create a finite precision element one can either cast from other types of elements,
or add O(p^n) to a varying precision element.

We haven't yet decided how equality will behave.

AUTHORS:
    - William Stein (2004): first version
    - David Joyner (2005-12-24): examples
    - David Roe (2006-10-7): reworked as part of Sage Days 2
"""
...
class pAdicRingElementGeneric(noetherian_ring_element.NoetherianRingElement):
    r"""
    The generic class for $p$-adic integers.
    """

class pAdicRingElementFinPrec(pAdicRingElementGeneric, discrete_valuation_ring_element.DiscreteValuationRingElement):
    r"""
    The class for $p$-adic integers with finite precision.

    EXAMPLES:
        sage: a = 1 + 17 + 3*17^2 + O(17^3); a
          1 + 17 + 3*17^2 + O(17^3)
        sage: b = 5015 + O(17^8); b
	  6*17 + 17^3 + O(17^8)
	sage: a + b
	  1 + 7*17 + 3 * 17^2 + O(17^3)
	sage: a * b
	  6*17 + 6*17^2 + 2*17^3 + O(17^4)
	sage: loads(a.dumps()) == a
        True

    """

    def __init__(self, parent, x, big_oh, ordp=None, construct=False):
        r"""
        INPUT:
            parent -- a p-adic field.
            x -- anything that can be coerced to a p-adic number.
            big_oh -- is such that, e.g. 1 + 2 * 3 + (3^2)
                       has big_oh equal to 2.
        """
#Implementation not yet fixed from William's code
    ...
    def ordp(self):
        ...
        """
#Need to implement something here because we don't guarantee that the internal state keeps u a unit.
        return self.__ordp
...
class pAdicRingElementArbPrec(pAdicRingElementGeneric, discrete_valuation_ring_element.DiscreteValuationRingElement):
    r"""
    The class for $p$-adic integers with arbitrary precision.

    EXAMPLES:
	sage: R = Zp(17)
	sage: x = R(6/23); x
	  17-adic element 6/23
	sage: x + O(17^4)
	  1 + 14*17 + 11*17^2 + 3*17^3 + O(17^4)
	sage: y = log(x); y
	  17-adic element log(6/23)
	sage: z = log(R(2/19))
	sage: y + z
	  17-adic element log(6/23) + log(2/19)
	sage: y + z + O(17^4)
	  5*17 + 5*17^2 + 6*17^3 + O(17^4)
    """

class pAdicRingElementRational(pAdicRingElementArbPrec):
    r"""
    The class for rational numbers inside $\Z_p$.

    EXAMPLES:
	sage: R = Zp(17)
	sage: x = R(3/4); x
	  17-adic integral element 3/4
	sage: x + O(17^4)
	  5 + 4*17 + 4*17^2 + 4*17^3 + O(17^4)
	sage: x.prec(4)
	  5 + 4*17 + 4*17^2 + 4*17^3 + O(17^4)
	sage: y = 17*x; y
	  17-adic integral element 51/4
	sage: y + O(17^4)
	  5*17 + 4*17^2 + 4*17^3 + O(17^4)
	sage: y.prec(4)
	  5*17 + 4*17^2 + 4*17^3 + 4*17^4 + O(17^5)
	sage: R(3/4,4)
	  5 + 4*17 + 4*17^2 + 4*17^3 + O(17^4)
	sage: R(51/4,4)
	  5*17 + 4*17^2 + 4*17^3 + 4*17^4 + O(17^5)
	sage: z = R(0); z
	  17-adic integral element 0
	sage: z.valuation()
	  infinity
	sage: a = exp(log(R(1+17^2))) - R(1+17^2); a
	  17-adic integral element exp(log(290)) - 290
	sage: a.valuation()
	  #problem....
    """

class pAdicRingElementFixMod(pAdicRingElementGeneric):
    r"""
    The class for $p$-adic integers represented as elements of $\Z / p^n \Z$.

    EXAMPLES:
	sage: R = ZpFixed(17,4); S = Zp(17)
	sage: x = R(3/4); x
	  17-adic fixed modulus integral element 5 + 4*17 + 4*17^2 + 4*17^3 + O(17^4)
	sage: y = 17^2 * x; y
	  17-adic fixed modulus integral element 5*17^2 + 4*17^3 + O(17^4)
	sage: z = y.shiftLeft(1); z
	  17-adic fixed modulus integral element 5*17 + 4*17^2 + O(17^3)
	sage: z.modulus()
	  3
	sage: z.parent().modulus()
	  4
	sage: z.prec()
	  2
    """
