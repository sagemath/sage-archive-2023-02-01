...
* Compute the cardinality of a (very large) set defined recursively (through a
  call to :class:`RecursivelyEnumeratedSet of forest type<SearchForest>`)
...
This implementation is specific to :class:`RecursivelyEnumeratedSet of forest
type<SearchForest>`, and uses its properties to do its job. Non only mapping
...
:class:`RecursivelyEnumeratedSet of forest type<SearchForest>` representing
your set `S` (see :mod:`sage.sets.recursively_enumerated_set`). Then, you
need to provide a Map function as well as a Reduce function. Here are some
examples :
...
* **Counting the number of elements**: In this situation, the map function
  ...
  values of the :meth:`SearchForest.map_reduce` method so that to compute the
