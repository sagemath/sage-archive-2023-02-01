r"""
Graded commutative algebras with maximal degree
...
class GradedCommutativeAlgebraWithMaxDeg(CombinatorialFreeModule, Algebra):
    ...
    A graded commutative algebra `A` with maximal degree is an integer-graded
    ring satisfying the super-algebra relation w.r.t. the degree modulo 2.
    ...
    where `n \in \NN \cup \{ \infty \}` is the maximal degree, and the
    ...
    `\mathrm{deg}(x_i)`. To that end, this algebra can be given a (dense) vector
    ...
    - ``degrees`` -- (optional) a tuple or list specifying the degrees
      ...
      raised.
    - ``max_degree`` -- (optional) the maximal degree of the graded algebra.
      If omitted, `\infty` is assumed.
    ...
    Graded commutative algebra with unlimited degree::

        sage: A.<x,y,z,t> = GradedCommutativeAlgebra(QQ, degrees=(1,2,2,3), max_degree=oo)
        sage: A.maximal_degree()
        +Infinity
        sage: list(A.basis().iterator_range(stop=17))
        [1, x, z, y, t, x*z, x*y, x*t, z^2, y*z, y^2, z*t, y*t, x*z^2, x*y*z, x*y^2, x*z*t]
    ...
    def __classcall_private__(cls, base, names=None, degrees=None,
                              max_degree=infinity, category=None, **kwargs):
        r"""
    ...
    def __init__(self, base, names, degrees, max_degree=infinity,
        ...
        """
        if max_degree != infinity and max_degree not in ZZ:
            raise TypeError('max_degree must be an integer or infinity')
