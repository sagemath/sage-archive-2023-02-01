...
class ScalarField(CommutativeAlgebraElement):
    ...
    Consequently, scalar fields can be added::
        ...
        f+c: M --> R
        on U: (x, y) |--> (a*x^2 + a*y^2 + a + 1)/(x^2 + y^2 + 1)
        on V: (u, v) |--> (a*u^2 + a*v^2 + u^2 + v^2 + a)/(u^2 + v^2 + 1)
    ...
    is performed only on the chart domain::
        ...
        sage: s.display()
        M --> R
        on V: (u, v) |--> (u^3 + u*v^2 + u^2 + v^2 + u)/(u^2 + v^2 + 1)
    ...
    domain of the result is then this subset::
        ...
        on W: (u, v) |--> (u^6 + 3*u^4*v^2 + 3*u^2*v^4 + v^6 + u^3*v + u*v^3 + u*v)/(u^6 + 3*u^4*v^2 + 3*u^2*v^4 + v^6 + u^4 + 2*u^2*v^2 + v^4)
    ...
    is the pointwise multiplication of functions::
        ...
        sage: s.display()
        f*f: M --> R
        on U: (x, y) |--> 1/(x^4 + 2*x^2*y^2 + y^4 + 2*x^2 + 2*y^2 + 1)
        on V: (u, v) |--> (u^4 + 2*u^2*v^2 + v^4)/(u^4 + 2*u^2*v^2 + v^4 + 2*u^2 + 2*v^2 + 1)
    ...
    defined on `U`::
        ...
        U --> R
        (x, y) |--> x*y/(x^2 + y^2 + 1)
        on W: (u, v) |--> u*v/(u^4 + 2*u^2*v^2 + v^4 + u^2 + v^2)
    ...
    Scalar fields can be divided (pointwise division)::
        ...
        sage: s.display()
        f/c: M --> R
        on U: (x, y) |--> 1/((x^2 + y^2 + 1)*a)
        on V: (u, v) |--> (u^2 + v^2)/((u^2 + v^2 + 1)*a)
        sage: s = g/h ; s
        Scalar field g/h on the Open subset U of the 2-dimensional topological
         manifold M
        sage: s.display()
        ...
        U --> R
        (x, y) |--> 1/((x^2 + y^2 + 1)*x*y)
        on W: (u, v) |--> (u^6 + 3*u^4*v^2 + 3*u^2*v^4 + v^6)/((u^2 + v^2 + 1)*u*v)
        sage: s == f.restrict(U)/g
        True
    ...
    def display(self, chart=None):
        ...
        def _display_expression(self, chart, result):
            ...
            """
            try:
                expression = self.coord_function(chart).expr('SR')
                coords = chart[:]
                if len(coords) == 1:
                    coords = coords[0]
