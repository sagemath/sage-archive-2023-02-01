...
# This borrows some interface ideas
# from a file written Peter Tingley and Adam Schultz
...
class PBWCrystalElement(Element):
    def __init__(self, parent, long_word, lusztig_datum):
        ...
        self.pbw_datum = PBWDatum(parent.pbw_datum_parent, long_word, lusztig_datum)

    def _repr_(self):
        """
        Return a string representation of ``self``.
        """
        pbw_datum = self.pbw_datum.convert_to_new_long_word(self.parent()._default_word)
    ...
    def _latex_(self):
        ...
        al = self.parent().pbw_datum_parent._root_list_from(self.parent()._default_word)
        from sage.misc.latex import latex
        latex_exp = lambda i: "^{%s}"%latex(i) if i > 1 else ""
        ret_str = ' '.join("f_{%s}%s"%(latex(al[i]), latex_exp(exp))
    ...
    def lusztig_datum(self, word=None):
        ...
        pbw_datum = self.pbw_datum.convert_to_new_long_word(word)
    ...
    def __eq__(self, other):
        ...
        """
        other_long_word = other.pbw_datum.long_word
        other_lusztig_datum = other.pbw_datum.lusztig_datum
        equiv_pbw_datum = self.pbw_datum.convert_to_new_long_word(other_long_word)
    ...
    def _cmp_(self, other):
        ...
        """
        word = self.parent().pbw_datum_parent._long_word_begin_with(1)
        lusztig_datum = tuple(self.pbw_datum.convert_to_new_long_word(word).lusztig_datum)
        other_lusztig_datum = tuple(other.pbw_datum.convert_to_new_long_word(word).lusztig_datum)
        return cmp(lusztig_datum, other_lusztig_datum)

    @cached_method
    def __hash__(self):
        i = self.parent().index_set()[0]
        word = self.parent().pbw_datum_parent._long_word_begin_with(i)
        pbw_datum = self.pbw_datum.convert_to_new_long_word(word)
    ...
    def f(self, i):
        ...
        EXAMPLES::
            ...
            True
        """
        equiv_PBWDatum = self.pbw_datum.convert_to_long_word_with_first_letter(i) 
    ...
    def e(self, i):
        equiv_pbw_datum = self.pbw_datum.convert_to_long_word_with_first_letter(i) 
    ...
    def star(self):
        ...
        EXAMPLES::
            ...
            True
        """
        starred_pbw_datum = self.pbw_datum.star()
    ...
    def weight(self):
        ...
        al = WLR.simple_roots()
        return WLR.sum(c*al[i] for i,c in self.pbw_datum.wt())
...
class PBWCrystal(Parent, UniqueRepresentation):
    ...
    def __init__(self, cartan_type):
        ...
        self.pbw_datum_parent = PBWData(self._cartan_type)
        ...
        self._default_word = self.pbw_datum_parent._long_word_begin_with(i)
