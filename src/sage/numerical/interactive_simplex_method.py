...
Historically it was created to complement Math 373 course on Mathematical
...
.. admonition:: Corn & Barley
    ...
    Corn has a net profit of 10 dollars per acre while barley has a net profit
    of 5 dollars per acre.
    The farmer has 1500 kg of fertilizer available with 3 kg per acre needed for
    corn and 1 kg per acre needed for barley.
...
def _latex_product(coefficients, variables,
    ...
    - ``separator`` -- (default: "&" with some extra space adjustment) a string
      to be inserted between elements of the generated expression;
      
    - ``head`` -- either ``None`` (default) or a list of entries to be added to
      the beginning of the output;
      
    - ``tail`` -- either ``None`` (default) or a list of entries to be added to
    ...
    - a strings joining ``head``, each sign and coefficient-variable product,
...
def variable(R, v):
    ...
    - ``v`` -- a variable of ``R`` or convertible into ``R``, a string
      with the name of a variable of ``R`` or an index of a variable in
      ``R``.
...
class LPProblem(SageObject):
    ...
    - ``x`` -- (default: "x") a vector of decision variables or a string giving
      the base name;
...
class LPProblemStandardForm(LPProblem):
    ...
    - ``slack_variables`` -- (default: same as ``x`` parameter, if it was given
      as a string, otherwise string "x") a vector of slack variables or a sting
      giving the base name;
      
    - ``auxiliary_variable`` -- (default: same as ``x`` parameter with adjoined
      "0" if it was given as a string, otherwise "x0") the auxiliary variable
      name, expected to be the same as the first decision variable for auxiliary
      problems;
    ...
    constraints (they are always "<=") and variables (they are always ">="), and
    the problem type may only be "max" or "-max". You may give custom names to
    slack and auxiliary variables, but in most cases defaults should work::
    ...
    def initial_dictionary(self):
        ...
        The initial dictionary "defines" :meth:`slack_variables` in terms of the
        :meth:`~LPProblem.decision_variables`, i.e. it has slack variables as
        basic ones.
    ...
    def revised_dictionary(self, *x_B):
        ...
        - basic variables for the dictionary to be constructed. If not given,
          ...
          :meth:`auxiliary_variable` to
          give a feasible dictionary.
    ...
    def run_simplex_method(self):        
        ...
        .. note::
            ...
            You can access the :meth:`final_dictionary`, which can be one of the
            following:
    ...
    def slack_variables(self):
        ...
        Slack variables are differences between the constant terms and left hand
...
class LPAbstractDictionary(SageObject):
    ...
    def is_feasible(self):
        ...
        - ``True`` if all :meth:`~LPDictionary.constant_terms` are non-negative, 
          ``False`` otherwise.
    ...
    def possible_simplex_method_steps(self):
        ...
        - a list of pairs ``(entering, leaving)``, where ``entering`` is a
          non-basic variable that may :meth:`enter` and ``leaving`` is a list of
          basic variables that may :meth:`leave` when ``entering`` enters. Note
          that ``leaving`` may be empty, indicating that the problem is
          unbounded.
...
class LPDictionary(LPAbstractDictionary):
    ...
    .. note::
        ...
        This constructor does not check correctness of input, as it is intended
        to be used internally by :class:`LPProblemStandardForm`.
    ...
    def __eq__(self, other):
        ...
        - ``True`` if ``other`` is an :class:`LPDictionary` with all details the
          same as ``self``, ``False`` otherwise.
...
#            lines[-1] += r"\require{color}"
    ...
    def ELLUL(self, entering, leaving):
        ...
        leaving variable is red in the original dictionary state on the top. The
        new state after the update step is shown on the bottom.
        """
        self.enter(entering)
        self.leave(leaving)        
...
class LPRevisedDictionary(LPAbstractDictionary):
    ...
    def E(self):
        ...
        If `B_\mathrm{old}` is the current matrix `B` and `B_\mathrm{new}` is
        the `B` matrix of the next dictionary (after the update step), then
    ...
    def y(self):
        r"""
        Return the `y` vector, the product of :meth:`c_B` and :meth:`B_inverse`.
