...
### Constructors
    ...
    def ring(self, base_ring='ZZ', vars='[x]', order='Lex'):
        ...
        of order 7, with graded reverse lex ordering:
            sage: R1 = macaulay2.ring('ZZ/7', '[a..d]', 'GRevLex'); R1
...
class Macaulay2Element(ExpectElement):
    ...
    def __floordiv__(self, x):
        ...
        EXAMPLE:
            sage: R = PolynomialRing(GF(7), 2, 'xy', macaulay2=True)
            sage: x, y = R.gens()
            ...
            x^21+2*x^7*y^14
            sage: f1 = (x^2 + 2*y*x)
            sage: h1 = macaulay2(f1)
            sage: f2 = (x^3 + 2*y*x)
            sage: h2 = macaulay2(f2)
            sage: h % [h1,h2]
            -3*x*y
            sage: u = h // [h1,h2]; u
    ...
    def __mod__(self, x):
        ...
        EXAMPLE:
            ...
            x^21+2*x^7*y^14
            sage: f1 = (x^2 + 2*y*x)
            sage: h1 = f1._macaulay2_()
            sage: f2 = (x^3 + 2*y*x)
            sage: h2 = f2._macaulay2_()
            sage: h % [h1,h2]
            -3*x*y
            sage: u = h // [h1,h2]
            sage: h == u[0]*h1 + u[1]*h2 + (h % [h1,h2])
