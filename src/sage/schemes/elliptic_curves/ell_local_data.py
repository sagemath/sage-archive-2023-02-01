...
def check_prime(K,P):
    ...
    if is_NumberFieldFractionalIdeal(P):
        if K.fractional_ideal(P).is_prime():
            return P
        else:
    ...
    if is_NumberFieldElement(P):
        if P in K:
            ...
            raise TypeError("The element %s is not an element of %s" % (P, K))
        if P.is_prime():
            return P
        else:
            raise TypeError("The ideal generated by %s is not a prime ideal of %s" % (P, K))
