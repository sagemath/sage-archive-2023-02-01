r"""
Plotting for 3D implicit surfaces (isosurfaces).

This implementation is based on Jmol's
src/org/jmol/jvxl/calc/MarchingCubes.java (and some bits, such as the
data tables, are directly copied from there).
...
#*****************************************************************************
# Pieces of this file are strongly based on, or directly copied from,
# src/org/jmol/jvxl/calc/MarchingCubes.java in the Jmol source code, so
# this copyright is inherited from that file.
...
cdef class MarchingCubes:
    ...
    3. Call finish() (returns a list of strings).
...
cdef class MarchingCubesTriangles(MarchingCubes):
    ...
    def process_slice(self, unsigned int x, np.ndarray slice):
        ...
        if x == 0:
            self.update_yz_vertices(x,
                                    None,
                                    self.slices[0],
                                    self.slices[1])

            self.update_x_vertices(x,
                                   None,
                                   self.slices[0],
                                   self.slices[1],
                                   self.slices[2] if len(self.slices) >= 3 else None)
    ...
    cpdef update_yz_vertices(self, int x, np.ndarray _prev, np.ndarray _cur, np.ndarray _next):
        """
        TESTS::
            ...
            sage: cube_marcher.update_yz_vertices(1, prev_slice, cur_slice, next_slice)
    ...
    cpdef update_x_vertices(self, int x, np.ndarray _prev, np.ndarray _left, np.ndarray _right, np.ndarray _next):
        """
        TESTS::
            ...
            sage: cube_marcher.update_x_vertices(1, prev_slice, left_slice, right_slice, next_slice)
    ...
    cpdef process_cubes(self, np.ndarray _left, np.ndarray _right):
        ...
        TESTS::
            ...
            sage: slices[0][1, 1] = -1
            sage: cube_marcher.update_yz_vertices(0, None, slices[0], slices[1])
            sage: cube_marcher.update_x_vertices(0, None, slices[0], slices[1], slices[2])
...
# Copied from MarchingCubes.java
