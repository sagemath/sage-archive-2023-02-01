...
def one_radical_difference_family(K, k):
    ...
    problem is solved by a call to the function :func:`one_cyclic_tiling`.
        ...
        observe that `\Delta B` is always a union of cosets of `C`.

        .. MATH::

            \begin{array}{llll}
            (k\text{ odd} ) & \Delta B &= \{r^i-r^j:r^i\neq r^j\}       &= C\cdot \{r^{i}-1:r^{i}\neq 1\}\\
            (k\text{ even}) & \Delta B &= \{r^i-r^j:r^i\neq r^j\}\cup C &= C\cdot \{r^{i}-1:r^{i}\neq 1\}\cup C
            \end{array}

        Consequently, `\mathcal B = \{x_1 B, \ldots, x_k B\}` is a radical
        difference family if and only if `\{x_1 (\Delta B/C), \ldots, x_k
        (\Delta B/C)\}` is a partition of the cyclic group `K^*/C`.
