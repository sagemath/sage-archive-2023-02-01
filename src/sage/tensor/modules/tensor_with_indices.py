...
class TensorWithIndices(SageObject):
    ...
    def __add__(self, other):
        ...
        EXAMPLES::
            ...
            sage: b = M.tensor((0,2), name='b')
            sage: b[:] = [[-1,2,-3], [-4,5,6], [7,-8,9]]
            sage: a["^(..)"] == 1/2*(a["^ij"]+a["^ji"])
            sage: (a*a)["^..(ij)"] == 1/2*((a*a)["^..ij"]+(a*a)["^..ji"])
        ...
        if set(self._con) != set(other._con) or set(self._cov) != set(other._cov):
            raise ValueError("Indices set are not identical")
        ...
        # -------------------------------------
        result = TensorWithIndices(self._tensor, self._con + '_' + self._cov)
        
        permutation_con = list(range(self._tensor.tensor_type()[0]))
        for self_index in range(self._tensor.tensor_type()[0]):
            if self._con[self_index] == other._con[self_index]:
                permutation_con[self_index] = self_index
            else:
                permutation_con[self_index] = other._con.index(self._con[index]) 
        ...
        swap_params = list(combinations(range(1, self._tensor.tensor_type()[0]+2), 3))
    
        # The associated permutation is as follows
        def swap(i,j,k,N):
            L = list(range(1,N+1))
            L = L[:i-1]+L[j-1:k-1]+L[i-1:j-1]+L[k-1:]
        return L
        
        # Construction of the permutation group generated by swaps
        perm_group = PermutationGroup(
            [swap(*param, self._tensor.tensor_type()[0]) for param in swap_params],
            canonalize = False
        )
        
        # Compute a decomposition of the permutation as a product of swaps
        decomposition_as_string = perm_group(permutation_con).word_problem(
            perm_group.gens(),
            display=False
        )[0]
        ...
        decomposition = [ (swap_params[int(x)-1],int(y)) for x,y in decomposition ]
       
        components = self._tensor.comp(fmodule._def_basis)
        for swap_param in decomposition:
            components.swap_adjacent_indices(*swap_param)
        
        permutation_cov = {}
        for self_index in range(self._tensor.tensor_type()[1]):
            if self._cov[self_index] == other._con[self_index]:
                permutation_con[self_index] = self_index
            else:
                permutation_cov[self_index] = other._cov.index(self._con[index]) 
