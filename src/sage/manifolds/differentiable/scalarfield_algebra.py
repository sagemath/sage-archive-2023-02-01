...
class DiffScalarFieldAlgebra(ScalarFieldAlgebra):
    ...
    def __init__(self, domain):
        ...
        # We need to redefine _name, _latex_name and __custom_name,
        # which has been set to those for algebras of continuous scalar fields
        # by ScalarFieldAlgebra.__init__ :
        degree = self._domain.diff_degree()
        if degree == infinity:
            latex_degree = r"\infty"  # to skip the "+" in latex(infinity)
        else:
            latex_degree = "{}".format(degree)
        self._name = "C^{}(".format(latex_degree) + self._domain._name + ")"
        self._latex_name =  r"C^{" + latex_degree + r"}\left("  + \
                            self._domain._latex_() + r"\right)"
        self.rename("Algebra of differentiable scalar fields on " + \
