...
the process of implementing a finite semigroup. Start by copy-pasting
the code of the class
:class:`sage.categories.examples.finite_semigroups.LeftRegularBand` into, say,
a cell of the notebook. and query-replace every occurence of
``FiniteSemigroups().example(...)`` in the documentation by LeftRegularBand.
...
Use ``TestSuite`` to test the newly implemented semigroup, and draw
its Cayley graph.
