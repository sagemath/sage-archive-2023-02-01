...
class QuotientModuleWithBasis(CombinatorialFreeModule):
    ...
    def __init__(self, submodule, category):
        ...
        TESTS::
            ...
            sage: Y.category()
            Join of Category of vector spaces with basis over Rational Field and Category of quotients of sets
            sage: Y.basis().list()
            [B[2]]
...
class SubModuleWithBasis(CombinatorialFreeModule):
    ...
    def __classcall_private__(cls, basis, ambient = None, category = None, *args, **opts):
        ...
        category = ModulesWithBasis(ambient.base_ring()).Subobjects().or_subcategory(category, join=True)
    ...
    def _inverse_on_support(self, j):
        return self._invsup.get(j, None)
    ...
    def retract(self):
        r"""
        Returns the retract map from the ambient space.
