...
class FiniteRankFreeModule(UniqueRepresentation, Parent):
    ...
    def isomorphism_with_fixed_basis(self, basis, codomain=None):
        ...
        EXAMPLES::
            ...
            sage: basis = e = V.basis("e"); e
            Basis (e_0,e_1,e_2) on the 3-dimensional vector space over the Rational Field
            sage: phi_e = V.isomorphism_with_fixed_basis(basis); phi_e
            Generic morphism:
              From: 3-dimensional vector space over the Rational Field
              To:   Free module generated by {'e_0', 'e_1', 'e_2'} over Rational Field
        ...
        TESTS::
            ...
            Traceback (most recent call last):
            ...
            ValueError: domain and codomain should have the same rank
        ...
        if codomain is None:
            codomain = CombinatorialFreeModule(base_ring, [b._name for b in basis])
        else:
            if codomain.rank() != self.rank():
                raise ValueError("domain and codomain should have the same rank")
        codomain_basis = list(codomain.basis())
        def function(x):
            return codomain.sum(x[basis, i] * codomain_basis[i] for i in self.irange())
        return ModuleMorphismFromFunction(self, function, codomain,
