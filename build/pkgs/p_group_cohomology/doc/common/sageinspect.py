...
- Modifications for usage in the p_group_cohomology spkg
...
import os
import tokenize
from sage.misc.sageinspect import sage_getfile as orig_getfile
from pGroupCohomology.cohomology import permanent_result

EMBEDDED_MODE = False

SPKG_ROOT = os.environ['SPKG_ROOT']

Mod2File = {'pGroupCohomology':'pGroupCohomology/__init__.py',
            'pGroupCohomology.barcode':'pGroupCohomology/barcode.py',
            'pGroupCohomology.cochain':'pGroupCohomology/cochain.pyx',
            'pGroupCohomology.cohomology':'pGroupCohomology/cohomology.pyx',
            'pGroupCohomology.dickson':'pGroupCohomology/dickson.pyx',
            'pGroupCohomology.factory':'pGroupCohomology/factory.py',
            'pGroupCohomology.modular_cohomology':'pGroupCohomology/modular_cohomology.pyx',
            'pGroupCohomology.mtx':'pGroupCohomology/mtx.pyx',
            'pGroupCohomology.resolution':'pGroupCohomology/resolution.pyx'}
...
def _sage_getargspec_cython(source):
    ...
    """
    try:
        return _sage_getargspec_from_ast(source)
    except SyntaxError:
        pass
    try:
...
def sage_getfile(obj):
    ...
    The following has been enabled in trac ticket #11791::
        ...
        ... '        return int(12345)']
        sage: cython('\n'.join(cython_code))
        sage: print ''.join(open(sage_getfile(Bar2)).readlines())
        <BLANKLINE>
    ...
    """
    from pGroupCohomology.cohomology import permanent_result
    if isinstance(obj, permanent_result):
        return sage_getfile(obj._f)
    ...
    try:
        ...
        try:
            mod = obj.__objclass__.__module__
        except AttributeError:
            mod = ''
    try:
        return os.path.realpath(os.path.join(SPKG_ROOT,'src',Mod2File[mod]))
    except KeyError, msg:
    ...
    if pos is not None:
        (_, filename, _) = pos
        # Verify that it is the correct file. Move on to a temporary file,
        # if it turns out that it can not be found in the sage library:
        try:
...
def find_first_line(filename,obj):
    if inspect.ismethod(obj) or inspect.ismethoddescriptor(obj):
        try:
            source_lines,offset = sage_getsourcelines(obj.im_class)
        except AttributeError:
            try:
                source_lines,offset = sage_getsourcelines(obj.__objclass__)
            except AttributeError:
                source_lines = open(filename).readlines()
                offset = 1
    else:
        source_lines = open(filename).readlines()
        offset = 1
    if isinstance(obj, type) and type(obj)!=type:
        try:
            baseobj = obj.__base__
            return find_first_line(filename, baseobj)
        except AttributeError:
            print "WARNING",obj
    for i,s in enumerate(source_lines):
        ...
        if inspect.isclass(obj):
            if s.startswith('class ') or s.startswith('cdef class '):
                if s.split('class ',1)[1].startswith(obj.__name__) and s.split(obj.__name__,1)[1][0] in ' :(':
                    return i+offset
        if inspect.isfunction(obj) or inspect.ismethod(obj) or inspect.ismethoddescriptor(obj) or inspect.isbuiltin(obj):
            if s.startswith('def '):
                if s.split('def ',1)[1].startswith(obj.__name__+' ') or s.split('def ',1)[1].startswith(obj.__name__+'('):
                    return i+offset
            elif s.startswith('cpdef '):
                if (' '+obj.__name__ +' ') in s or (' '+obj.__name__+'(') in s:
                    return i+offset
    return offset
...
def sage_getsourcelines(obj, is_binary=False):
    ...
    We show some enhancements provided by trac ticket #11768::

        sage: P = JackPolynomialsP(QQ)
        sage: sage_getsourcelines(P.element_class)
    ...
    """
    if isinstance(obj,permanent_result):
        return sage_getsourcelines(obj._f)
    try:
        mod = obj.__module__
    except AttributeError:
        mod = ''
    ...
    try:
        orig_filename = orig_getfile(obj)
    except:
        orig_filename = ''
    if filename != orig_filename: # we are in our spkg
        lineno = find_first_line(filename,obj)
        try:
            source_lines = open(filename).readlines()
        except IOError:
            return None
    ...
    from sage.misc.sageinspect import sage_getsourcelines as orig_getsourcelines
