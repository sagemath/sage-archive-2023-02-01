...
http://www.risc.uni-linz.ac.at/people/hemmecke/AldorCombinat/combinatse10.html. One
notable difference is that we use power-sum symmetric functions as the coefficients
of our cycle index series.
...
def OrdinaryGeneratingSeriesRing(R):
    """
    Returns the ring of ordinary generating series.  Note that is is just a
...
class ExponentialGeneratingSeries(LazyPowerSeries):
    ...
    def functorial_composition(self, y):
        ...
        $g = \sum_{n=0}^{\infty} f_n \frac{x^n}{n!}$, then functorial
        composition $f \Box g$ is defined as
    ...
    def _functorial_compose_gen(self, y, ao):
        """
        Returns a generator for the coefficients of the functorial composition
        of self with y.
...
class CycleIndexSeries(LazyPowerSeries):
    def count(self, t):
        """
        Returns the number of structures corresponding to a certain
        cycle type.
    ...
    def stretch(self, k):
        ...
        If $ f = \sum_{n=0}^{\infty} f_n(x_1, x_2, \ldots, x_n) $, then
        the stretch $g$ of $f$ by $k$ is
    ...
    def _ogs_gen(self, ao):
        """
        Returns a generator for the coefficients of the ordinary generating series
        obtained from a cycle index series.
    ...
    def _egs_gen(self, ao):
        """
        Returns a generator for the coefficients of the exponential generating series
        obtained from a cycle index series.
    ...
    def functorial_composition(self, g):
        ...
        If $F$, $G$, and $H$ are combinatorial species such that
        $ H = F \Box G $, then the cycle index series $Z_H = Z_F \Box Z_G$
        is defined as
    ...
    def _functorial_compose_gen(self, g, ao):
        """
        Returns s generator for the coefficients of the functorial composition of
        self with g.
    ...
    def _card(self, n):
        """
        Returns the number of structures on an underlying set of size n for the species
        associated with self.  This is just n! times the coefficient of p[1]^n in self.
    ...
    def _compose_term(self, p, y_powers):
        ...
        INPUT:
            p -- a term in self
            y_powers -- a stream for the powers of y starting
                        with y
    ...
    def weighted_composition(self, y_species):
        """
        Returns the composition of this cycle index series with the cycle index
        series of  y_species where y_species is a weighted species. Note that
        this is basically the same algorithm as composition except we can't  use
        the optimization that the powering of cycle index series commutes
        with 'stretching'.
    ...
    def _weighted_compose_gen(self, y_species, ao):
        """
        Returns an iterator for the composition of this cycle index series and
        the cycle index series of the weighted species y_species.
