...
cdef class pAdicZZpXFMElement(pAdicZZpXElement):
    ...
    cdef int _set_from_ZZ_pX_c(self, ZZ_pX_c poly, ntl_ZZ_pContext_class ctx) except -1:
        """
        prime_pow should be set before calling this function.
        """
        if not ctx.modulus().is_power_of(self.prime_pow.prime):
            raise ValueError, "ZZ_pX element must have prime power modulus over the same prime."
        #self.prime_pow.restore_top_context()
        ZZ_pX_conv_modulus(self.value, poly, self.prime_pow.get_top_context().x)
