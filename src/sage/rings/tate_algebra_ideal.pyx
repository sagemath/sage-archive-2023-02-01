...
and, in particular, provides an implementation of Buchberger
...
cdef _groebner_basis_buchberger(I, prec, bint integral):
    ...
    if base.is_field():
        if integral:
            # We need to minimize and reduce the Gröbner basis again
            i = 0
            while i < len(rgb):
                ti = (<TateAlgebraElement>rgb[i])._terms_c()[0]
                for j in range(len(rgb)):
...
class TateAlgebraIdeal(Ideal_generic):
    ...
    def groebner_basis(self, prec=None, algorithm='buchberger-integral'):
        ...
        The Gröbner basis `(g_1,\dots,g_n)` of this ideal, uniquely determined
        ...
        The algorithm ``buchberger-integral`` first compute a Gröbner basis 
