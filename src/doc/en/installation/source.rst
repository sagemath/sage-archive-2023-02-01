...
To fix it create a temporary build folder in the Linux file system using `mkdir -p ~/tmp/sage` and use it for building by `./configure --prefix=/home/<USER>/tmp/sage`, then run `make` again.
