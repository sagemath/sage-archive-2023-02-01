...
class OpenInterval(DiffManifold):
    ...
    ``I`` is a 1-dimensional differentiable manifold over `\RR`::
        ...
        sage: isinstance(I, DiffManifold)
        True
        sage: I.base_field()
        'real'
        sage: dim(I)
    ...
    :meth:`~sage.categories.sets_cat.Sets.SubcategoryMethods.Facade`)::
        ...
        sage: K.category()
        Category of facade sets
        sage: I.category()  # I has not been created as a subinterval
        Category of sets

    Consequently, the parent of their elements is the whole interval from
    which they have been created (``I`` in the present case)::
