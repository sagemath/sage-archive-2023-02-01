...
class Link:
    ...
    def vogel_move(self):
        ...
        if bad_region == []:
            ...
            #adding new elements to the bad regions
            k = []
            for i in pd_copy:
                k.append(max(i))
            new_component_1 = [min(bad_region), max(k) + 1, max(k) + 2]
            new_component_2 = [max(bad_region), max(k) + 3, max(k) + 4]
            print new_component_1
