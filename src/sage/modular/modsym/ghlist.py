...
class GHlist_old:
    r"""
    Deprecated class, kept only minimally for pickling old pickles.

    TESTS::

        sage: L = sage.modular.modsym.ghlist.GHlist_old(GammaH(18,[13]))
        sage: loads(dumps(L)) == L
        True
    """
    def __init__(self, group):
    ...
    def __eq__(self, other):
        ...
        EXAMPLES::
            ...
            True
        """
        if not isinstance(other, GHlist_old):
            return False
    ...
    def convert(self):
        """
        Conversion to the new-style class :class:`GHlist_new`.

        EXAMPLES::

            sage: L = sage.modular.modsym.ghlist.GHlist_old(GammaH(18,[13]))
            sage: L.convert()
            List of coset representatives for Congruence Subgroup Gamma_H(18)
            with H generated by [13]
        """
        return GHlist_new(self.__group)


register_unpickle_override('sage.modular.modsym.ghlist', 'GHlist', GHlist_old)
