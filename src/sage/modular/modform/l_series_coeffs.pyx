...
from sage.rings.all import ZZ, PowerSeriesRing, O as bigO
...
cpdef to_series(L, var):
    ...
    if var is None:
        ...
        return PowerSeriesRing(ZZ, var, len(L))(L)
    else:
        R = var.parent()
        return R(L) + bigO(R.gen()**len(L))


# TODO, when quadratic form code stabilizes, add this there.
def bqf_theta_series(Q, long bound, var=None):
    r"""
    Return the theta series associated to the given positive definite quadratic
    form. For a given form $f = ax^2 + bxy + cy^2$ this is the sum
