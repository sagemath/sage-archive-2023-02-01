...
class OrlikTeraoAlgebra(CombinatorialFreeModule):
    ...
    The *Orlik-Terao algebra* `A(M)` is the quotient `E / J(M)`. This is
    a graded finite-dimensional commutative `R`-algebra. The non-broken
    circuit (NBC) sets of `M` (that is, the subsets of `X` containing
    no broken circuit of `M`) form a basis of `A(M)`. (Recall that a
    *broken circuit* of `M` is defined to be the result of removing
