...
class HighestWeightCrystals(Category_singleton):
    ...
    class ElementMethods:
        ...
        def all_paths(self,index_set=None,start=[]):
            r"""
            ...
            - ``self`` -- an element of a crystal

            - ``index_set`` -- a subset of the index set of ``self`
            ...
            EXAMPLES::
                ...
                sage: L = b.all_paths()
                sage: L
                <generator object all_paths at 0x1183a0690>
                sage: list(L)
                [[2, 1, 2, 1], [2, 2, 1, 1]]
            ...
            for i in index_set:
                next = self.e(i)
                if next is not None:
                    for x in next.all_paths(index_set,start+[i]):
                        yield x
                    hw = False
            if hw:
                yield start
