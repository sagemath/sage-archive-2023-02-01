...
*Splitting algebras* have been considered by Dan Laksov, Anders Thorup, Torsten Ekedahl and others (see references below) in order
to study intersection theory of Grassmann and other flag schemes. Similarily as *splitting fields* they can be considered as
extensions of rings containing all the roots of a given monic polynomial over that ring under the assumption that its Galois
group is the symmetric group of order equal to the polynomial's degree.

Thus they can be used as a tool to express elements of a ring generated by `n` indeterminates in terms of symmetric functions
in these indeterminates.

This realization of splitting algebras follows the approach of a recursive quotient ring construction splitting off some linear
factor of the polynomial in each recursive step. Accordingly it is inherited from :class:`PolynomialQuotientRing_domain`.
...
class SplittingAlgebraElement(PolynomialQuotientRingElement):
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.functional import cyclotomic_polynomial
    ...
    def __invert__(self):
        r"""
        overloads inherited method in order to support inversion of special elements attached
        to the construction of the parent and which are recorded in the list
...
class SplittingAlgebra(PolynomialQuotientRing_domain):
    r"""
    For a given monic polynomial `p(t)` of degree `n` over a commutative ring `R`, the
    splitting algebra is the universal `R`-algebra in which `p(t)` has `n` roots, or,
    more precisely, over which `p(t)` factors,

    .. MATH::

        p(t) = (t - \xi_1 ) \cdots (t - \xi_n).

    This class creates an algebra as extension over the base ring of a given polynomial
    `p` such that `p` splits into linear factors over that extension. It is assumed
    (and not checked in general) that the Galois group of `p` is the symmetric Group
    `S(n)`. The construction is recursive (following [LT2012]_, 1.3).

    INPUT:

    - ``monic_polynomial`` -- the monic polynomial which should be split
    - ``names``  -- names for the indeterminates to be adjoined to the base ring of
      ``monic_polynomial``
    - ``warning`` -- (optional boolean, default = True) can be used (by setting to False)
      to suppress a warning which will be thrown whenever it cannot be checked that
      the Galois group of ``monic_polynomial`` is maximal
    ...
    def __init__(self, monic_polynomial, names='X', iterate=True, warning=True):
        ...
        verbose("Create splitting algebra to base ring %s and polynomial %s (%s %s)" %(base_ring, monic_polynomial, iterate, warning))

        self._defining_polynomial = monic_polynomial
        self._iterate      = iterate

        try:
            if base_ring.is_integral_domain() == False:
                raise TypeError( "base_ring must be an integral domain" )
        except NotImplementedError:
            from sage.rings.ring import Ring
            if isinstance(base_ring, Ring) == False:
    ...
    def __reduce__(self):
        ...
        EXAMPLES::
            ...
            sage: S = SplittingAlgebra(x^4 -t*x^3 - u*x^2 - v*x + w, ('X', 'Y', 'Z'))
            sage: S.__reduce__()
    ...
    def _repr_(self):
        ...
        if self.is_completely_split():
            return 'Splitting Algebra of %s with roots %s over %s' %(self.defining_polynomial(), self.splitting_roots(), self.scalar_base_ring())
        else:
            return 'Factorization Algebra of %s with roots %s over %s' %(self.defining_polynomial(), self.splitting_roots(), self.scalar_base_ring())
    ...
    def _first_ngens(self, n):
        ...
        EXAMPLES::
            ...
            sage: X.parent()
            Splitting Algebra of x^3 + (-u^2 + v)*x^2 + (u + v)*x - 1 with roots [X, Y, -Y - X + u^2 - v]
            over Multivariate Polynomial Ring in u, v over Integer Ring
        """
    ...
    def is_completely_split(self):
        ...
        EXAMPLES::
            ...
            False
        """
        if len(self.splitting_roots()) < self.defining_polynomial().degree():
            return False
        return True
...
def solve_with_extension(monic_polynomial, root_names=None, var='x', flatten=False, warning=True):
    ...
    - ``var``  -- (optional string, default 'x') for the indeterminate needed to define the
      splitting field of the ``monic_polynomial`` (if necessary and possible)
    - ``flatten`` -- (optional boolean, default = True) if set True the roots will not be
      given as a list of pairs ``(root, multiplicity)`` but as a list of roots repeated according
    ...
    List of tuples ``(root, multiplicity)`` respectively list of roots repeated according
    to their multiplicity if option ``flatten`` is ``True``.
    ...
    def create_roots(monic_polynomial, warning=True):
        r"""
        This internal function creates all roots of a polynomial in an appropriate extension ring
        assuming that none of the roots is contained its base ring.

        It first tries to create the splitting field of the given polynomial. If this is not
        faithful the splitting algebra will be created.
        ...
        try:
            ...
            if   isinstance(base_ring, NumberField_generic):
                reset_coercion = True
            elif base_ring.is_finite() == True and base_ring.is_prime_field() == False:
                reset_coercion = True
            if reset_coercion == True:
