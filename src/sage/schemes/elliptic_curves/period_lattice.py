...
from sage.rings.all import ZZ, QQ, RealField, ComplexField, QQbar, AA, CDF
...
################################################################################
# Double precision analytic computation of the period mapping (modular symbol)
# associated to an elliptic curve, without doing linear algebra.
################################################################################


def period_matrices(E, tau):
    """
    Return the period matrices of E at the cusp tau ?

    EXAMPLES::

        sage: from sage.schemes.elliptic_curves.period_lattice import period_matrices
        sage: E = EllipticCurve('11a1')
        sage: period_matrices(E,0)
        (2, [[1 0]
        [0 1], [1 0]
        [0 1], [ 6  1]
        [11  2]])
    ...
    N = E.conductor()
    # 1. find a prime p that is suitable, along with matrices M[i]
    for p in Primes():
        if N % p == 0:
            continue
        # are the cusps tau, p*tau, and (tau+j)/p for j=0,...,p-1 all equivalent?
        t = Cusps(tau)
        M = []
...
def modsym(E, tau, prec=53, terms=100):
    ...
    This does not use any linear algebra!

    The result is phi(tau) = sum(omega(m) for m in M)/(1+p-a_p), where

    EXAMPLES::

        sage: from sage.schemes.elliptic_curves.period_lattice import modsym
        sage: E = EllipticCurve('17a1')
        sage: modsym(E,0)  # abs tol 1e-10
...
def period(E, M, prec=53, abs_prec=10):
    """
    omega(m) = 2*pi*I*integral_{tau,M(tau)} f(z) dz

    See pages 35-36, Section 2.10 of Cremona's book for how to do this,
    in particular, Prop. 2.10.3.

    EXAMPLES::

        sage: from sage.schemes.elliptic_curves.period_lattice import period
        sage: E = EllipticCurve('19a1')
        sage: M = matrix(ZZ,[[10,1],[19,2]])
        sage: period(E, M)  # abs tol 1e-10
        1.35975973351 - 1.09779285698e-16*I
    """
    eps = -E.root_number()
    N = E.conductor()
    if M[1, 1] < 0:
    ...
    R = RealField(prec)
    delta = R(10)**(-abs_prec)
    terms = (-R(d)*R(N).sqrt()*delta.log() / (2*R.pi())).ceil()

    from sage.misc.all import verbose
    tm = verbose("computing %s terms of anlist..." % terms)
    an = E.anlist(terms+1)
    tm = verbose("got anlist; now computing formal sum", tm)
    for n in range(1, terms+1):
        npi2 = 2*pi*n
        w = (-npi2/(d*sqrtN)).exp()
        t = (K(an[n])/K(n))*((eps-1)*(-npi2/sqrtN).exp() +
                             w * ((i*npi2*b/d).exp() - eps*(i*npi2*c/d).exp()))
        s += t
    verbose("computed sum", tm)
    return -s   # minus because cremona's I_f is backwards from ours

def modular_symbol_numerical(self, sign, prec=53, abs_prec=10):
    """
