...
class MPL_Point: public Py::PythonExtension<MPL_Point> {
public:
  MPL_Point(LazyValue* x, LazyValue*  y);
  ~MPL_Point();
  static void init_type(void);
...
public:
  Bbox(MPL_Point* ll, MPL_Point* ur);
  ~Bbox();
  static void init_type(void);
  ...
  */
  ...
  MPL_Point* ll_api() {return _ll;}
  MPL_Point* ur_api() {return _ur;}
private:
...
public:
    ...
    : Py::ExtensionModule<_transforms_module>( "_transforms" )
  {
    ...
    add_varargs_method("Value", &_transforms_module::new_value,
		       "Value(x)");
    add_varargs_method("MPL_Point", &_transforms_module::new_point,
		       "MPL_Point(x, y)");
