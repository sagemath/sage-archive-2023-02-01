...
class TensorWithIndices(SageObject):
    ...
    def __add__(self, other):
        ...
        EXAMPLES::
            ...
            True
            sage: (a*a)["^..(ij)"]["abji"] == 1/2*((a*a)["^abij"]+(a*a)["^abji"])
            True
        ...
        TESTS::
            ...
            sage: b[:] = [[-1,2,-3], [-4,5,6], [7,-8,9]]
            sage: T = a*a*b*b
            sage: 1/4*(T["jikl_abcd"] + T["ijkl_abdc"] + T["jikl_abdc"]+T["ijkl_abcd"] ) == T["(..).._..(..)"]["ijkl_abcd"]
            True
        """
        ...
        permutation = list(range(self._tensor.tensor_rank()))
        for self_index in range(self._tensor.tensor_type()[0]):
            if self._con[self_index] == other._con[self_index]:
                permutation[self_index] = self_index
            else:
                permutation[self_index] = other._con.index(self._con[self_index]) 
        for self_index in range(self._tensor.tensor_type()[1]):
            if self._cov[self_index] == other._cov[self_index]:
                permutation[self._tensor.tensor_type()[0] + self_index]\
                    = self._tensor.tensor_type()[0] + self_index
            else:
                permutation[self._tensor.tensor_type()[0] + self_index]\
                    = self._tensor.tensor_type()[0] + other._cov.index(self._cov[self_index]) 
        # ~ print(permutation)
        ...
        for swap_param, exponent in decomposition:
            if exponent > 0 :
                ...
                pass
        result = other.__pos__()
        result._tensor = result._tensor + self._tensor._fmodule.tensor_from_comp(
