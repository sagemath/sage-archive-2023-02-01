...
class PermutationGroup_generic(group.FiniteGroup):
    ...
    @cached_method
    def _transitive_subgroup_gap(self, order):
        r"""
        Return a conjugacy class of transitive subgroups of the given order,
        if there is one, as a GAP element.
        ...
        The symmetric group on 4 elements has a transitive subgroup of order 4::

            sage: S4 = groups.permutation.Symmetric(4)
            sage: S4._transitive_subgroup_gap(4)
            ConjugacyClassSubgroups(SymmetricGroup( [ 1 .. 4 ] ),Group( 
            [ (1,4)(2,3), (1,3)(2,4) ] ))

        However, it does not have a transitive subgroup of order 6::

            sage: S4 = groups.permutation.Symmetric(4)
            sage: S4._transitive_subgroup_gap(6) is None
            True
        ...
        """ % (self._gap_().name(), order, len(self.domain())))
    ...
    @cached_method
    def transitive_subgroup(self, order = None, return_group = True):
        r"""
        Return a transitive subgroup of the given order if there is one.
        ...
        - ``return_group`` (boolean) -- If ``return_group = True`` (default),
          a transitive subgroup of the given order is returned if there is
          one, and ``None`` if there isn't. When ``return_group = False``,
          only a boolean indicating whether such a group exists is returned
          instead.
        ...
        b = False
        G = None
        if order >= len(self.domain()) and self.order() % order == 0:
            if self.order() == order:
                b = self.is_transitive()
                if b:
                    G = self
            else:
                C = self._transitive_subgroup_gap(order)
                b = (C is not None)
