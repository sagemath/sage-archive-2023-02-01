...
Let `A` be a commutative ring with unity.  The *Yangian* `Y(\mathfrak{gl}_n)`,
associated with the Lie algebra `\mathfrak{gl}_n` for `n\ge 1`, is defined to
be the unital associative algebra generated by
`\{t_{ij}^{(r)} : 1 \le i,j \le n , r \ge 1\}` subject to the relations
...
where `L,M \ge 0` and `t_{ij}^{(0)} = \delta_{ij}\cdot 1`.  This system of
quadratic relations is equivalent to the system of commutation relations

.. MATH::

    [t_{ij}^{(r)}, t_{k\ell}^{(s)}] = \sum_{p=0}^{\min\{r,s\}-1}
