...
class ProductProjectiveSpaces_morphism_ring(SchemeMorphism_polynomial):
    ...
    def is_morphism(self):
        ...
        the domain of ``self`` we consider the subscheme generated by the corresponding set of coordinates of ``self``.
        ``self`` is a morphism iff each of these subschemes has no points.
        ...
        ::
            ...
            sage: P.<x,y,z,w,u,v> = ProductProjectiveSpaces([2,2],QQ)
            sage: H = End(P)
            sage: f = H([x^2*u,y^2*w,z^2*v,w^2,u^2,v^2])
            sage: f.is_morphism()
            False
        """
        m = 0
        T = self.domain().ambient_space()
        for i in range(T.num_components()):
            t = T[i].dimension_relative() + 1
