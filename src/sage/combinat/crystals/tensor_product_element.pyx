...
cdef class TensorProductOfQueerSuperCrystalsElement(TensorProductOfRegularCrystalsElement):
    ...
    Given crystals `B_1` and `B_2` of type `Q(n+1)`, we define the tensor product 
    `b_1\otimes b_2\in B_1\otimes B_2` where `b_1\in B_1` and `b_2\in B_2` as the following:
 
    In addition to the tensor product rule for type `A_n`, the tensor product rule for `e_{-1}` 
    and `f_{-1}` on `b_1\otimes b_2` are given by
    ...
    \begin{cases}
    b_1 \otimes e_{-1}b_2, \quad if \text{wt}(b_1)_1 = \text{wt}(b_1)_2 = 0,\\
    e_{-1}b_1 \otimes b_2, \quad \text{otherwise},
    \end{cases}\\

    f_{-1}(b_1\otimes b_2) = 

    \begin{cases}
    b_1 \otimes f_{-1}b_2, \quad if \text{wt}(b_1)_1 = \text{wt}(b_1)_2 = 0,\\
    f_{-1}b_1 \otimes b_2, \quad \text{otherwise}.
    \end{cases}
    \end{aligned}
    ...
    Here, `w_i=s_2\ldots s_i s_1\ldots s_{i-1}` and `s_i` is the reflection along the `i`-string 
    in the crystal. Moreover, for `1<i\leq n`, we define the operators e_{-i'} and f_{-i'} as

    .. MATH::

    e_{-i'} = s_{w_0} e_{-i} s_{w_0}, \quad f_{-i'} = s_{w_0} f_{-i} s_{w_0},

    where `w_0` is the longest element in the symmetric group `S_{n+1}` generated by `s_1,\ldots,s_n`.
    In this implementation, we use the integers -2n,\ldots,-(n+1) to respectively denote the 
    indices -n',\ldots,-1'.
