...
def eigenmatrix(int v,int k,int l,int mu):
    ...
    matrix).  Thus they can be simultaneously diagonalized. Hence `A` and `A'` share
    eigenspaces. The eigenvalues of `J` are `v` with multiplicity 1, and 0
    with multiplicity `v-1`. Thus the eigenvalue of `A'` corresponding to
    the 1-dimension `k`-eigenspace of `A` is `v-k-1`.  Respecively, the
    eigenvalues of `A'` corresponding to `t`-eigenspace of `A`, with `t` unequal to
    `k`, equals `-t-1`. The 1st eigenmatrix `P` of the C-algebra `C[A]` generated by `A`
    encodes this eigenvalue information in its three colums; the 2nd (resp. 3rd) column
    contains distinct eigenvalues of `A` (resp. of `A'`), and the 1st column contains
    the corresponding eigenvalues of `I`. The matrix `vP^{-1}` is called the 2nd eigenvalue
    matrix of `C[A]`.
    The most interesting feasure of `vP^{-1}` is that it is the 1st eigenmatrix of
    the dual  of `C[A]` --- if the dual is generated by the adjacency matrix of a
    ...
    An s.r.g. with a non-isomorphic dual coming from an s.r.g.::
...
def SRG_243_110_37_60():
    ...
    .. NOTE::
        ...
        is from a projective 2-intersection set.
