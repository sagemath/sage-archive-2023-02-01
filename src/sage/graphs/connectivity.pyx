...
def staticSPQRTree(G):
    r"""
    Construct a static spqr tree using the output generated by triconnectivity
    which gives bond, polygon and triconnected components present in graph `G`.
    Iterate over all the components and create respective Tree nodes, based on
    component edges and graph `G` edges construct edges between two tree nodes.
    First construct the tree with 0..n-1 labeled vertices then replace them
    with original vertex labels.

    EXAMPLES::

        sage: from sage.graphs.connectivity import staticSPQRTree, spqr_tree_to_graph
        sage: G = Graph(2)
        sage: for i in range(3):
        ....:     G.add_clique([0, 1, G.add_vertex(), G.add_vertex()])
    ...
    for i in range(len(tric.comp_list_new)):
        ...
        # Do this case exist?
        if not len(tric.comp_list_new[i]):
            continue

        # Create a vertex in tree
        treeVertex = Tree.add_vertex()

        if tric.comp_type[i] == 0:
            int_to_treeVertex.append(("P", Graph(tric.comp_list_new[i], immutable=True, multiedges=True)))

        elif tric.comp_type[i] == 1:
            int_to_treeVertex.append(("S", Graph(tric.comp_list_new[i], immutable=True, multiedges=True)))
